...
declare function simc:edit-distance ( $s1 as xs:string, $s2 as xs:string ) as xs:integer {
 if(fn:string-length($s1) = 0) then fn:string-length($s2) else
 if(fn:string-length($s2) = 0) then fn:string-length($s1) else
 fn:min((
  simc:edit-distance(fn:substring($s1, 2), $s2) + 1 ,
  simc:edit-distance($s1, fn:substring($s2, 2)) + 1 ,
  simc:edit-distance(fn:substring($s1, 2), fn:substring($s2, 2)) + ( if(fn:substring($s1, 1, 1) = fn:substring($s2, 1, 1)) then 0 else 1 )
 ))
};
...
declare function simc:jaro ( $s1 as xs:string, $s2 as xs:string ) as xs:double {
 let $s    := for $i in ($s1,$s2) order by fn:string-length($i) return $i
 let $l1   := fn:string-length($s[1])
 let $l2   := fn:string-length($s[2])
 let $mt   := xs:integer((fn:max(($l1,$l2)) div 2.0) - 1)
 let $mc   := for $i in 1 to fn:min( ($l1 , $l2) ) 
              let $auxmatch := fn:substring($s[2], fn:max((1,$i - $mt)), $mt * 2 )
              return for $j in 1 to fn:string-length($auxmatch)  
                     where fn:substring($auxmatch, $j, 1) = fn:substring($s[1], $i, 1)
                     return <match char="{fn:substring($s[1], $i, 1)}" pos1="{$i}" pos2="{$j + fn:max((1,$i - $mt)) - 1}" />
 let $m    := if (fn:count($mc) = 0) then (1) else (fn:count($mc))
 let $t    := fn:count( for $i in $mc, $j in $mc where $i/@pos1>$j/@pos1 and $i/@pos2<$j/@pos2 return $i )
...
declare function simc:jaro-winkler ( $s1 as xs:string, $s2 as xs:string, $prefix as xs:integer, $fact as xs:double ) as xs:double {
 ...
 let $cc   := for $i in 1 to fn:min(($prefix, fn:string-length($s1), fn:string-length($s2))) 
              where fn:substring($s1, 0, $i) = fn:substring($s2, 0, $i) return $i
 return ($jaro + ( $fact * fn:max($cc) * ( 1 - $jaro ) ) )
};
...
declare function simc:needleman-wunsch ( $s1 as xs:string, $s2 as xs:string, $score as xs:integer, $penalty as xs:integer ) as xs:double{
 ...
 if(fn:string-length($s1) = 0) then fn:string-length($s2)* - $penalty else
 if(fn:string-length($s2) = 0) then fn:string-length($s1)* - $penalty else
 fn:max((
  simc:needleman-wunsch(fn:substring($s1, 2), $s2, $score, $penalty) - $penalty ,
  simc:needleman-wunsch($s1, fn:substring($s2, 2), $score, $penalty) - $penalty ,
  simc:needleman-wunsch(fn:substring($s1, 2), fn:substring($s2, 2), $score, $penalty) + ( if(fn:substring($s1, 1, 1) = fn:substring($s2, 1, 1)) then $score else -$penalty )
 ))
};
...
declare function simc:smith-waterman ( $s1 as xs:string, $s2 as xs:string, $score as xs:integer, $penalty as xs:integer ) as xs:double{ 
 if(fn:string-length($s1) = 0) then 0 else
 if(fn:string-length($s2) = 0) then 0 else
 fn:max((
  0,
  simc:smith-waterman(fn:substring($s1, 2), $s2, $score, $penalty) - $penalty ,
  simc:smith-waterman($s1, fn:substring($s2, 2), $score, $penalty) - $penalty ,
  simc:smith-waterman(fn:substring($s1, 2), fn:substring($s2, 2), $score, $penalty) + ( if(fn:substring($s1, 1, 1) = fn:substring($s2, 1, 1)) then $score else -$penalty )
 ))
};
